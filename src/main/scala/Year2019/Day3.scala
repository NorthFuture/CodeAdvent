...
object Day3 extends App {
  ...
  def parseWire(wire: String) = {
    wire
      .split(",")
      .map(x => (x.head, x.tail.toInt))
  }
  val grid = mutable.Map[(Int, Int), Map[Char, Int]]()

  def printGrid() = {
    val minX = grid.keys.minBy(_._1)._1
    val maxX = grid.keys.maxBy(_._1)._1

    val minY = grid.keys.minBy(_._2)._2
    val maxY = grid.keys.maxBy(_._2)._2

    def getPointRapresentation(x: Int, y: Int): Char = {
      val wires = grid.getOrElse((x, y), Map())
    ...
    val display = minY - 5 to maxY + 5 map { y =>
      minX - 5 to maxX + 5 map { x =>
        getPointRapresentation(x, y)
      } mkString ("")
    } mkString ("\n")
    println(display)
  }
  ...
  @tailrec
  def drawWire(wire: Array[(Char, Int)],
               wireId: Char,
               pos: (Int, Int),
               currentSteps: Int): Unit = {

    if (wire.nonEmpty) {
      val action = wire.head._1
      val steps = wire.head._2

      def drawPoint(pos: (Int, Int), currentSteps: Int): Unit = {
        grid(pos) = grid.getOrElse(pos, Map()) + ((wireId, currentSteps))
      }

      val movements = action match {
        case 'R' =>
          pos._1 to pos._1 + steps map ((_, pos._2))
        case 'L' =>
          pos._1 to pos._1 - steps by -1 map ((_, pos._2))
        case 'D' =>
          pos._2 to pos._2 + steps map ((pos._1, _))
        case 'U' =>
          pos._2 to pos._2 - steps by -1 map ((pos._1, _))
      }

      val endState = movements.foldLeft((movements.head, currentSteps)) {
        (state, x) =>
          drawPoint(state._1, state._2)
          (x, state._2 + 1)
      }

      drawWire(wire.tail, wireId, endState._1, endState._2 - 1)
    }
  }

  drawWire(wire1, 'o', (0, 0), 0)

  drawWire(wire2, '*', (0, 0), 0)

//  printGrid()

  val intersections = grid
    .filter(x => x._2.size > 1)
    .filter(x => x._1 != (0, 0))

  val part1 = intersections.map(x => math.abs(x._1._1) + math.abs(x._1._2)).min
  val part2 = intersections.map(x => x._2.map(_._2).sum).min - 2
